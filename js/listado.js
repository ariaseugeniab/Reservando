...
Listado.prototype.buscarRestaurante = function(id) {
    ...
    // if(this.restaurantes.find(elem => elem.id === id)){
    //     return this.restaurantes.find(elem => elem.id === id);
    // }
    // return "No se ha encontrado ningún restaurant";

    let resto = this.restaurantes.find(elem => elem.id === id);

    //return resto? resto : "No se ha encontrado ningún restaurant";
    return resto || "No se ha encontrado ningún restaurant";
}
...
Listado.prototype.obtenerUbicaciones = function() {
    ...
    // }
    this.restaurantes.map(x => ciudades.push(x.ubicacion));
    //Se crea un nuevo array donde se van a agregar las ciudades pero sin repetirse
    // var ciudadesFiltadas = ciudades.filter(function(elem, index, self) {
    //     return index === self.indexOf(elem);
    // });
    var ciudadesFiltadas = this.filtrarRepetidos(ciudades)
    return ciudadesFiltadas.sort();
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    // }
    this.restaurantes.map(x => rubros.push(x.rubro));

    // var rubrosFiltrados = rubros.filter(function(elem, index, self) {
    //     return index === self.indexOf(elem);
    // });

    var rubrosFiltrados = this.filtrarRepetidos(rubros)
...
Listado.prototype.obtenerHorarios = function() {
    ...
    // for (var i = 0; i < this.restaurantes.length; i++) {
    //     horariosArray.push(this.restaurantes[i].horarios);
    // }
    this.restaurantes.map(x => horariosArray.push(x.horarios));

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios= [];
    horariosArray.forEach((a) => {
        a.forEach((hor) => {
            horarios.push(hor)
        });
    });
    ...
    //     return index === self.indexOf(elem);
    // });
    var horariosFiltrados = this.filtrarRepetidos(horarios);
...
Listado.prototype.filtrarRepetidos = function(arreglo){
    let result = arreglo.filter((elem, index, self) => {
        return index === self.indexOf(elem);
    });
    return result
}
